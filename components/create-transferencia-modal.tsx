...
const basicFormSchema = z.object({
  cd_destino: z.string().min(1, 'CD de destino é obrigatório'),
...
export function CreateTransferenciaModal({ isOpen, onClose, onSuccess }: CreateTransferenciaModalProps) {
  ...
  const { createTransferencia, isLoading, error } = useTransferencias()
   ...
   <Dialog open={isOpen} onOpenChange={handleClose}>
     <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
       <DialogHeader>
         ...
         <DialogDescription>
           Crie uma nova transferência de ativos entre centros de distribuição
         </DialogDescription>
       </DialogHeader>
         ...
         <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             <div className="space-y-2">
               ...
               <Input value={user?.cd || ""} disabled />
             </div>

             <div className="space-y-2">
               <Label htmlFor="cd_destino">CD de Destino</Label>
               <Select 
                 value={form.watch('cd_destino')} 
                 onValueChange={(value) => form.setValue('cd_destino', value)}
               >
                 <SelectTrigger>
                   <SelectValue placeholder="Selecione o CD de destino" />
                 </SelectTrigger>
                 <SelectContent>
               ...
               {form.formState.errors.cd_destino && (
                 <p className="text-sm text-destructive">
                   {form.formState.errors.cd_destino.message}
                 </p>
               )}
             </div>
           </div>
