...
interface CreateRegressoModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: () => void
}

export function CreateRegressoModal({ isOpen, onClose, onSuccess }: CreateRegressoModalProps) {
  ...
  const { createRegresso, isLoading, error } = useRegressos()
  const { token } = useAuth()
 ...
 return (
   <Dialog open={isOpen} onOpenChange={handleClose}>
     <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
       <DialogHeader>
         <DialogTitle>Novo Regresso</DialogTitle>
         <DialogDescription>
           Crie um novo regresso de ativos de uma loja para o CD
         </DialogDescription>
       </DialogHeader>
       ...
       {loadingTipos ? (
         ...
         <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
           <div className="space-y-2">
             <Label htmlFor="loja_origem">Loja de Origem</Label>
             <Select 
               value={form.watch('loja_origem')} 
               onValueChange={(value) => form.setValue('loja_origem', value)}
             >
               <SelectTrigger>
             ...
             {form.formState.errors.loja_origem && (
               <p className="text-sm text-destructive">
                 {form.formState.errors.loja_origem.message}
               </p>
             )}
           </div>
           ...
           <div className="space-y-2">
             ...
             <Textarea
               id="observacoes"
               placeholder="Observações sobre o regresso..."
